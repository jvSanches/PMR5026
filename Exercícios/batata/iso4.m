classdef iso4 < handle
    ...
    methods
        ...
        function [K,M, index1, index2, index3, index4] = decomposeMatrices(obj)
        ...
        function obj = setPressure(obj, npx, npy, side)
            
            no1 = obj.n1;
            no2 = obj.n2;
            no3 = obj.n3;
            no4 = obj.n4;
            nmo1 = 0; %(npy) * obj.L^2 / 12; 
            
            switch side(end)
                case 1
                    L = sqrt((no2.x - no1.x)^2 + (no2.y - no1.y)^2) * obj.T;
                    obj.n1.setLoad(L * npx/2, L * npy/2, nmo1);
                    obj.n2.setLoad(L * npx/2, L * npy/2, -nmo1);
                case 2
                    L = sqrt((no3.x - no2.x)^2 + (no3.y - no2.y)^2)* obj.T;
                    obj.n3.setLoad(L * npx/2, L * npy/2, nmo1);
                    obj.n2.setLoad(L * npx/2, L * npy/2, -nmo1);
                case 3
                    L = sqrt((no4.x - no3.x)^2 + (no4.y - no3.y)^2)* obj.T;
                    obj.n4.setLoad(L * npx/2, L * npy/2, nmo1);
                    obj.n3.setLoad(L * npx/2, L * npy/2, -nmo1);
                 case 4
                    L = sqrt((no1.x - no4.x)^2 + (no1.y - no4.y)^2)* obj.T;
                    obj.n4.setLoad(L * npx/2, L * npy/2, nmo1);
                    obj.n1.setLoad(L * npx/2, L * npy/2, -nmo1);                    
            end
